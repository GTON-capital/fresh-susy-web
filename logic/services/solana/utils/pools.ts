...
import { cache, getCachedAccount, getMultipleAccounts } from "./accounts"
...
export const removeLiquidity = async (connection: Connection, wallet: any, liquidityAmount: number, account: TokenAccount, pool?: PoolInfo) => {
  ...
  const toAccounts: PublicKey[] = [
    await findOrCreateAccountByMint(wallet.publicKey, wallet.publicKey, instructions, cleanupInstructions, accountRentExempt, accountA.info.mint, signers),
    await findOrCreateAccountByMint(wallet.publicKey, wallet.publicKey, instructions, cleanupInstructions, accountRentExempt, accountB.info.mint, signers),
  ]

  const isLatestSwap = isLatest(pool.raw.account)
  const transferAuthority = approveAmount(instructions, cleanupInstructions, account.pubkey, wallet.publicKey, liquidityAmount, isLatestSwap ? undefined : authority)
  ...
  return [accountA.info.mint.equals(WRAPPED_SOL_MINT) ? (wallet.publicKey as PublicKey) : toAccounts[0], accountB.info.mint.equals(WRAPPED_SOL_MINT) ? (wallet.publicKey as PublicKey) : toAccounts[1]]
}

export const removeExactOneLiquidity = async (connection: Connection, wallet: any, account: TokenAccount, liquidityAmount: number, tokenAmount: number, tokenMint: string, pool?: PoolInfo) => {
  if (!pool) {
    throw new Error("Pool is required")
  }
  ...
  const toAccount: PublicKey = await findOrCreateAccountByMint(wallet.publicKey, wallet.publicKey, instructions, cleanupInstructions, accountRentExempt, tokenMatchAccount.info.mint, signers)
...
export const swap = async (connection: Connection, wallet: any, components: LiquidityComponent[], SLIPPAGE: number, pool?: PoolInfo) => {
  ...
  let toAccount = findOrCreateAccountByMint(wallet.publicKey, wallet.publicKey, instructions, cleanupInstructions, accountRentExempt, new PublicKey(components[1].mintAddress), signers)

  const isLatestSwap = isLatest(pool.raw.account)
  // create approval for transfer transactions
  const transferAuthority = approveAmount(instructions, cleanupInstructions, fromAccount, wallet.publicKey, amountIn, isLatestSwap ? undefined : authority)
  if (isLatestSwap) {
    signers.push(transferAuthority)
  }

  let hostFeeAccount = SWAP_HOST_FEE_ADDRESS ? findOrCreateAccountByMint(wallet.publicKey, SWAP_HOST_FEE_ADDRESS, instructions, cleanupInstructions, accountRentExempt, pool.pubkeys.mint, signers) : undefined
...
export const addLiquidity = async (connection: Connection, wallet: any, components: LiquidityComponent[], slippage: number, pool?: PoolInfo, options?: PoolConfig, depositType: string = "both") => {
...
//         .filter((item) => item.account.data.length === TokenSwapLayout.span || item.account.data.length === TokenSwapLayoutV1.span || item.account.data.length === TokenSwapLayoutV0.span)
//         .map((item) => {
//           let result = {
...
//           const layout = item.account.data.length === TokenSwapLayout.span ? TokenSwapLayout : item.account.data.length === TokenSwapLayoutV1.span ? TokenSwapLayoutV1 : TokenSwapLayoutV0
...
//         const [account0, account1] = await Promise.all([cache.queryAccount(connection, p.pubkeys.holdingAccounts[0]), cache.queryAccount(connection, p.pubkeys.holdingAccounts[1])])
...
//         const sorted = Object.entries(poolQuantities).sort(([pool0Idx, amount0], [pool1Idx, amount1]) => (amount0 > amount1 ? -1 : 1))
...
async function _addLiquidityExistingPool(pool: PoolInfo, components: LiquidityComponent[], connection: Connection, wallet: any) {
  ...
  const isLatestSwap = isLatest(pool.raw.account)
  // create approval for transfer transactions
  const transferAuthority = approveAmount(instructions, cleanupInstructions, fromKeyA, wallet.publicKey, amount0, isLatestSwap ? undefined : authority)
  if (isLatestSwap) {
    signers.push(transferAuthority)
  }
...
async function _addLiquidityExactOneExistingPool(pool: PoolInfo, component: LiquidityComponent, connection: Connection, wallet: any) {
  notify({
    message: "Adding Liquidity...",
    description: "Please review transactions to approve.",
    type: "warn",
  ...
  const reserve = accountA.info.mint.toBase58() === from.mintAddress ? accountA.info.amount.toNumber() : accountB.info.amount.toNumber()
  ...
  const accountRentExempt = await connection.getMinimumBalanceForRentExemption(AccountLayout.span)
  const fromKey = getWrappedAccount(instructions, cleanupInstructions, from.account, wallet.publicKey, amount + accountRentExempt, signers)
  ...
  const isLatestSwap = isLatest(pool.raw.account)
  // create approval for transfer transactions
  const transferAuthority = approveAmount(instructions, cleanupInstructions, fromKey, wallet.publicKey, amount, isLatestSwap ? undefined : authority)
  if (isLatestSwap) {
    signers.push(transferAuthority)
  }
...
function estimateProceedsFromInput(inputQuantityInPool: number, proceedsQuantityInPool: number, inputAmount: number): number {
  return (proceedsQuantityInPool * inputAmount) / (inputQuantityInPool + inputAmount)
}

function estimateInputFromProceeds(inputQuantityInPool: number, proceedsQuantityInPool: number, proceedsAmount: number): number | string {
...
export async function calculateDependentAmount(connection: Connection, independent: string, amount: number, pool: PoolInfo, op: PoolOperation): Promise<number | string | undefined> {
  const poolMint = await cache.queryMint(connection, pool.pubkeys.mint)
  const accountA = await cache.queryAccount(connection, pool.pubkeys.holdingAccounts[0])
...
async function _addLiquidityNewPool(wallet: any, connection: Connection, components: LiquidityComponent[], options: PoolConfig) {
  ...
  const [authority, nonce] = await PublicKey.findProgramAddress([tokenSwapAccount.publicKey.toBuffer()], programIds().swap)
  ...
  // creating depositor pool account
  const depositorAccount = createSplAccount(instructions, wallet.publicKey, accountRentExempt, liquidityTokenMint.publicKey, wallet.publicKey, AccountLayout.span)
  ...
  const feeAccount = createSplAccount(instructions, wallet.publicKey, accountRentExempt, liquidityTokenMint.publicKey, SWAP_PROGRAM_OWNER_FEE_ADDRESS || wallet.publicKey, AccountLayout.span)

  // create all accounts in one transaction
  let tx = await sendTransaction(connection, wallet, instructions, [liquidityTokenMint, depositorAccount, feeAccount, ...holdingAccounts, ...signers])
