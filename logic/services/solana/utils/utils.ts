...
export function getTokenName(map: KnownTokenMap, mintAddress: string, shorten = true, length = 5): string {
...
export function getTokenIcon(map: KnownTokenMap, mintAddress: string): string | undefined {
...
export function convert(account?: TokenAccount | number, mint?: MintInfo, rate: number = 1.0): number {
  ...
  const amount = typeof account === "number" ? new BN(account) : account.info.amount

  const precision = new BN(10).pow(new BN(mint?.decimals || 0))

  // avoid overflowing 53 bit numbers on calling toNumber()
  const div = amount.div(precision).toNumber()
  const rem = amount.mod(precision).toNumber() / precision.toNumber()
  const result = (div + rem) * rate
...
const format = (val: number, precision: number, abbr: boolean) => (abbr ? abbreviateNumber(val, precision) : val.toFixed(precision))

export function formatTokenAmount(account?: TokenAccount, mint?: MintInfo, rate: number = 1.0, prefix = "", suffix = "", precision = 6, abbr = false): string {
  if (!account) {
    return ""
  }
